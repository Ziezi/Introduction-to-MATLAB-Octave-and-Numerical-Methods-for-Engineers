...
function [Y] = Chapter14Exercise8 (x, y, X, D)
% Objective: Cubic Spline Interpolation with slope at end points specified by argument value.
% Input:
...
%   D - slopes of interpolating polynomials at end points. (S1'(x1) = D, Sn-1'(xn) = D)
% Output:
...
% Date: 27.09.2017  
  ...
  b = ConstructB(y, numberOfTerms, D);
...
%   i - number of polynomial which coefficients approximate the target value.
  ...
  indxs = find (data <= target);                             % find indexes of elements larger than curren value for interpolation.
  i = indxs(end);                                                    %  such that: x(i1) < X(i) <= x(i2).

end  

function [b] = ConstructB(y, numberOfTerms, D)
% Input:
...
%   b - r.h.s of the of the system of linear equations in matrix form.
  ...
  b = [b, zeros(1, (numberOfTerms * numberOfPolynomials) - numel(b) - 2), D, D]'; % specify values of derivatives at end points.
end  
...
%   A - NxN matrix consisted of the terms of the polynomials after the constraints have been applied.
  ...
  for i = 1 : len - 1                                                 % only internal common points of polynomials, not end points.
   ...
   coefficients = ones(1, numTerms );                % Apply constraints: S1''(x1) = 0 and Sn''(xn+1) = 0.
   dS =  PolyDerivative(coefficients);
   
   A(end - 1, :) = [zeros( 1, numTerms * (0) ),  shift(EvaluateTermsAt( x(1), dS ), -1), zeros( 1, numTerms * (len - 1) )];
   A(end, :) = [zeros( 1, numTerms * (len - 1) ),  shift(EvaluateTermsAt( x(end), dS ), -1), zeros( 1, numTerms * (0) ) ];
