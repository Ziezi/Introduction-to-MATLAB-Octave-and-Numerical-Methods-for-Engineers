% Function [Y] = MyCubicSplineFlat(x, y, X)

function [Y] = Chapter14Exercise5 (x, y, X)
% Objective: Cubic Spline (Flat) Interpolation (constraints on end points equal to 0).
% Input:
...
% Constraints on end points S'1(x_1) = 0 and S'n-1(x_n) = 0, instead of the second derivatives as in Exercise 2.
...
%   A - NxN matrix consisted of the terms of the polynomials after the constraints have been applied.
  ...
  for i = 1 : len - 1                                                 % only internal common points of polynomials, not end points.
   ...
   A(end - 1, :) = [zeros( 1, numTerms * (0) ),  shift(EvaluateTermsAt( x(1), dS ), -2), zeros( 1, numTerms * (len - 1) )];
   A(end, :) = [zeros( 1, numTerms * (len - 1) ),  shift(EvaluateTermsAt( x(end), dS ), -2), zeros( 1, numTerms * (0) ) ];
